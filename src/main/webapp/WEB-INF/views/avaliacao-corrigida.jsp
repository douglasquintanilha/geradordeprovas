...
<body>
	...
	<div class="container">
		...
		<h3>Sua nota foi:</h3>
		<h4>${relatorio.nota}/${avaliacao.quantidadeDeQuestoes}</h4>
		<h4>Gabarito:</h4>
		<c:forEach items="${avaliacao.questoes}" var="questao" varStatus="i">
			<h3>
				<md:render options="FencedCodeBlocks">${i.index+1} - ${questao.titulo}</md:render>
			</h3>
			<c:forEach items="${questao.alternativa}" var="alternativa">
					...
					<div class="radio alternativa">
						...
						<c:forEach items="${relatorio.alternativasMarcadas}" var="alternativaMarcada">
